...
class Ui_uygulamasec(object):
    ...
    def yaz(self):
        ...
        im=vt.cursor()
        im.execute("""DROP TABLE IF EXISTS uygulama""")   # tablo her çalışmayla birlikte siliniyor.
        im.execute("""CREATE TABLE uygulama("uygulamaadi")""") # tablo sıfırdan oluşturuluyor.bir sütunlu
        im.execute("""INSERT INTO uygulama(uygulamaadi) values(?)""",[(uygulama_adi)]) # uygulama adı yanlız başına gönderilemiyor.list şeklinde gitmesi lazım
        vt.commit() # bu olmadan prog. ismi veritabanına eklenmiyor
        im.execute("""SELECT * FROM uygulama""") #sorgu
        for i in im.fetchall(): # yazdırma amaçlı
            print(i)
        vt.close()
        sys.exit()

    '''
        command = "python3 giris.py"
        calistir = subprocess.Popen(command,shell=True)
